...
class OAuth2 {
	...
	public function getAuthorizeParams(array $inputData = NULL) {
		...
		// @see http://tools.ietf.org/html/draft-ietf-oauth-v2-20#section-4.1.2.1
		// @see http://tools.ietf.org/html/draft-ietf-oauth-v2-20#section-4.2.2.1
        // todo: add compare on redirect_uri
	 ...
	 * @ingroup oauth2_section_4
	 */
	public function finishClientAuthorizationAndRedirect($is_authorized, $user_id = NULL, $params = array()) {

        $result = $this->finishClientAuthorization($is_authorized, $user_id, $params);
		$this->doRedirectUriCallback($result["redirect_uri"], $result);
	}
    ...
    public function finishClientAuthorization($is_authorized, $user_id = NULL, $params = array())
    {
		...
		$state = $this->safe('state', $params);
        $scope = $this->safe('scope', $params);

        $result = array("redirect_uri"=>$redirect_uri);
    ...
    protected function safe($key, $array, $default = null)
    {
